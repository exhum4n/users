...
class VerificationService
{
    ...
    public function sendAuthorizationLink(string $email): void
    {
    ...
    public function confirmCode(string $email, int $code): User
    {
        ...
        if (is_null($verificationCode)) {
            throw new VerificationException(trans('auth.codeIsOutdated'), 404);
        }
    ...
    public function confirmEmail(string $code): User
    {
        ...
        if (is_null($email)) {
            throw new VerificationException(trans('auth.codeIsOutdated'));
        }
    ...
    protected function handleVerificationFail(string $email): void
    {
        ...
        if ($failAttempts >= self::MAX_FAIL_ATTEMPTS) {
            ...
            throw new VerificationException(trans('auth.throttle'));
        }

        $this->failRepository->setFailCount($email, $failAttempts);

        throw new VerificationException(trans('auth.wrongCode'));
    }
